...
def evaluate_model(model, X, y, patient_ids):
    ...
    for fold, (train_idx, val_idx) in enumerate(
        ...
        model.fit(X_train_fold, y_train_fold, validation_data=(X_val_fold, y_val_fold), epochs=10, verbose=1,
...
def main():
    ...
    hyperparams_set = transformer_hyperparameters_0

    # model = initialize_custom_model()
    model = initialize_Transformer_model(hyperparams_set)
    results = evaluate_model(model, X, y, patient_ids)
    timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
    save_results(results, f'evaluation_results_loocv_transformer_{timestamp}.pkl')
