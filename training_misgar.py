...
from tcam_model import CustomModelArchitecture
...
def train_custom_model(X, y, patient_ids, callbacks, sequence_length):
    ...
    for train_patient_ids, val_patient_ids in tqdm(kfold.split(unique_patient_ids), total=kfold.get_n_splits(),
        ...
        model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=100, verbose=1, callbacks=callbacks)
        val_loss, val_accuracy = model.evaluate(X_val, y_val, verbose=0)
        print(f"Validation accuracy: {val_accuracy:.4f}", f"Validation loss: {val_loss:.4f}")
